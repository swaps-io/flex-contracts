...
contract FlexConfirmNativeFacet is IFlexConfirmNative {
    ...
    function flexConfirmNative(
        bytes32 paramBundle0_, // Content: deadline (48), nonce (48), receiver (160)
        bytes32 paramBundle1_, // Content: amount (256)
        bytes32 paramBundle2_, // Content: key hash (256)
        bytes32 paramBundle3_, // Content: key (256)
        ...
        require(paramBundle2_ == keccak256(abi.encode(paramBundle3_)), FlexKeyError());

        bytes32[] memory componentHashes = new bytes32[](2);
        componentHashes[0] = keccak256(abi.encode(_receiveDomain, paramBundle0_, paramBundle1_)); // Receive native component
        componentHashes[1] = keccak256(abi.encode(_domain, paramBundle2_)); // Confirm native component
        ...
        uint96 nonce = uint48(uint256(paramBundle0_) >> 160);
        ...
        Address.sendValue(payable(receiver), uint256(paramBundle1_));
