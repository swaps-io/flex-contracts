...
import {
  calcFlexSendNativeBranch,
  calcFlexSendNativeHash,
  calcFlexTree,
  calcFlexTreeHash,
  encodeFlexSendNativeData0,
  encodeFlexSendNativeData1,
  encodeFlexSendNativeData2,
...
describe('FlexSendNativeFacet', function () {
  async function deployFixture() {
    ...
    const flexSendNativeDomain = '0xabababababababababababababababababababababababababababababababab';
    ...
    if (INSIDE_DIAMOND) {
      ...
      flexSendNativeFacet = await viem.deployContract('FlexSendNativeFacet', [flexSendNativeDomain]);
      flexSendNativeDomainFacet = await viem.deployContract('FlexSendNativeDomainFacet', [flexSendNativeDomain]);
  ...
  it('Should send native', async function () {
    ...
    const sendData0 = encodeFlexSendNativeData0({
      start,
      duration,
      sender,
    });
    const sendData1 = encodeFlexSendNativeData1({
      group,
      nonce,
      receiver,
    });
    const sendData2 = encodeFlexSendNativeData2({
      amount,
    });
    const sendHash = calcFlexSendNativeHash({
      domain: sendDomain,
      data0: sendData0,
      data1: sendData1,
      data2: sendData2,
    ...
    const sendComponentBranch = calcFlexSendNativeBranch({
      tree: orderTree,
